...
- `tools/blender/humanoid.py` + `specs.py` + `character.py`: construtor genérico (espelha a CharacterFactory: bodyType, cabelo, barba, óculos, terno/gravata, capa, armas, acessórios, olhos/boca) e os 8 clipes padrão; `blender -b -P tools/blender/character.py -- <tipo> public/models/<tipo>.glb`. **Os 16 tipos têm GLB em `public/models/`** (3,6 MB); `assetManager.enabled = false` (window.assetManager) força o procedural — os testes de lógica usam isso. Dino: peças `JUR_*` ocultas + materiais `SKIN_*` → `GLBCharacterVisual.transform('jurassic')` mostra, pinta de verde e cresce `jurassicScale`.
- Teste: `test/glb.mjs` (pula se não houver GLB; cobre tint por time, flash, altura, anda/ataca/morre, os 5 modelos e o Jurássico). Capturas: `test/shots/glb/`. Próximos: refinar silhuetas com as referências do Sol em `art_ref/<personagem>/` (os cabelos "capacete" e as proporções são o que mais precisa de olho humano); gerar os GLBs do Elenco 2 e adicionar em `AssetManager.MODEL_PATHS`.
...
- Philipe: "esta bom mas parece muito genérico" → segunda passada: `tools/blender/v2_face.py` (rosto compartilhado: olhos com íris/pupila/pálpebra `squint`/`tilt`/`gaze`, sobrancelhas `angle` (>0 bravo), nariz com narinas, boca `frown/smirk/smile/yell`, orelhas). Atenção ao sinal: rotação Y positiva desce o ponto de +x, por isso `rot_y = -sx*angle` para "canto interno para baixo". Cada personagem ganhou pose própria construída na geometria (os clipes giram o Empty do braço inteiro, então a pose sobrevive à animação): Barbudo de punhos à frente e barbão; Militante sem boné (cabelo cacheado alto é a identidade), bigodão, bandeira lisa `TEAM_flag`; Maconheiro com a mão esquerda no bolso, direita com o cigarrinho no queixo e fumaça subindo AO LADO da cabeça (na frente do rosto parecia um olho solto).
