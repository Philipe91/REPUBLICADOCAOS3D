...
- `tools/blender/humanoid.py` + `specs.py` + `character.py`: construtor genérico (espelha a CharacterFactory: bodyType, cabelo, barba, óculos, terno/gravata, capa, armas, acessórios, olhos/boca) e os 8 clipes padrão; `blender -b -P tools/blender/character.py -- <tipo> public/models/<tipo>.glb`. **GLBs no jogo: militante, barbudo, capitao, careca, dino** (os demais seguem procedurais até gerar). Dino: peças `JUR_*` ocultas + materiais `SKIN_*` → `GLBCharacterVisual.transform('jurassic')` mostra, pinta de verde e cresce `jurassicScale`.
